...
import { Heart, ShoppingCart } from "lucide-react";
 ...
 async function ProductDetail({
  ...
  const { title, price, image, colors, description } =
    await searchParams
 return (
    ...
    <div>
      <section className="w-full bg-[#FFFFFF] flex  px-4 lg:px-[59px] items-center justify-center mt-10">
        ...
        <div className="ml-20 ">
          ...
          <h6 className="font-medium text-sm text-gray-600 mb-3">{description}</h6>
          <p className="font-normal text-sm text-gray-800 mb-3">
            {price}
          </p>
         ...
         <Link href={`/cart?name=${title}&price=${price}&image=${image}&description=${description}`}>
